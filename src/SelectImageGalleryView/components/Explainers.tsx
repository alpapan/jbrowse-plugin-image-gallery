...
// Image gallery content component
const ImageGalleryContent = observer(function ImageGalleryContent({
  ...
  const parseImages = useCallback((): ImageData[] => {
    ...
    if (feature) {
      const images = feature.get('image') ?? feature.get('images')
  ...
  const isGroupExpanded = (groupLabel: string): boolean => {
    return expandedGroups.get(groupLabel) ?? true
  }
...
export const SelectImageGalleryViewF: React.FC<ImageGalleryViewProps> =
  observer(({ model }: ImageGalleryViewProps) => {
      ...
      return (
        ...
        >
          <Typography variant="h6" color="textSecondary">
            No feature with image data selected
          </Typography>
          <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>
            When you select a feature with associated images, they will appear
            here
          </Typography>
        </Paper>
      )
    ...
    // Show placeholder message when feature is selected but has no image data
    if (!model.featureImages || !model.hasContent) {
      return (
        <Paper sx={{ p: 2, m: 1 }} elevation={1}>
          ...
          >
            <Typography variant="body1" gutterBottom>
              Feature has no associated images
            </Typography>
            <Typography variant="body2">
              You can provide images via image_urls in the GFF file
            </Typography>
          </Box>
        </Paper>
      )
...
export default SelectImageGalleryViewF
